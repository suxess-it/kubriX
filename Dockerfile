...
# k8sgpt
COPY --from=ghcr.io/k8sgpt-ai/k8sgpt:v0.4.27 /k8sgpt /usr/local/bin/k8sgpt
...
# install CA crt
RUN mkdir -p /etc/tls && \
  printf '%s' "$KIND_KUBRIX_TLS_CRT_PEM" > /etc/tls/kind-kubrix-root-tls.crt && \
  printf '%s' "$KIND_KUBRIX_TLS_KEY_PEM" > /etc/tls/kind-kubrix-tls.key
